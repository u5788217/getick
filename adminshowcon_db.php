...
include 'config.php';
   ...
   $roundid = $_GET['round'];
	foreach ($oConn->query("SELECT * FROM round WHERE id_round = '$roundid'") as $aRow) {
		...
		echo '<h2>'.$aRow['date'].'<br> Time: '.$aRow['time'].'</h2><table border=1 style="width:300px; margin-left: 320px;">
                            <tr>
                                <td>ZONE</td>
		...
		case 1:case 2:case 6:case 10:case 14:case 15:
					...
					case 2:case 4:case 6:case 8:
						...
						foreach ($oConn->query("SELECT * FROM seat WHERE id_zone = '$zoneid[1]'") as $aRow) {
							...
							if($aRow['id_booking'] == NULL){
								echo '<td id="B">B'.$aRow['id_seat'].'<br><input type="checkbox" name="ticket[]" value="'.$aRow['id_seat'].' '.$zoneid[1].'" onchange="showCheck(this.value)" ></td>';
							}   
							...
							$j=$aRow['id_seat'];
							if($aRow['id_booking'] == NULL){
								echo '<td id="C">C'.$aRow['id_seat'].'<br><input type="checkbox" name="ticket[]" value="'.$aRow['id_seat'].' '.$zoneid[2].'" onchange="showCheck(this.value)" ></td>';							
}
